...
There are a few ways to create a new MIDI message.  Here are some examples
  ...
  with(:channel => 0, :velocity => 100) { |context| context.note_on("E4") }
...
As with any message, you can begin with raw data
...
No matter which of these methods you use, you will wind up with a pair of objects like this
...
The parse method will take any valid message data and return the corresponding object

  MIDIMessage.parse(0x90, 0x40, 0x40)
  
  MIDIMessage.parse(0xF0, 0x41, 0x10, 0x42, 0x12, 0x40, 0x00, 0x7F, 0x00, 0x41, 0xF7)
  
For more advanced MIDI parsing, see {nibbler}[http://github.com/arirusso/nibbler]  
